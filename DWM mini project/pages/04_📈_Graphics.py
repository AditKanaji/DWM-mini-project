...
# ===== html/css config ===== #
st.set_page_config(layout="wide", page_title="Graphics section")
...
# ===== Page ===== #
st.markdown('<p class="grand_titre">Graphics</p>', unsafe_allow_html=True)
st.write("##")
if 'data' in st.session_state:
    ...
    with col1:
        with st.expander("Data"):
            st.session_state.abscisse_plot = st.selectbox('Data in abscissa', col_num,
                                                          )
            st.session_state.ordonnee_plot = st.selectbox('Data in ordinates', col_num[::-1],
                                                          )
    with col2:
        with st.expander("Chart type"):
            st.session_state.type_plot = st.radio(label="label",
                                                  options=['Points', 'Curve', 'Latitude/Longitude', 'Histogram'],
                                                  help="Choose the type that suits you",
    ...
    if st.session_state.abscisse_plot and st.session_state.ordonnee_plot:
        if st.session_state.type_plot == 'Latitude/Longitude':
            ...
            if len(df_sans_NaN) == 0:
                st.warning('The dataset composed of the 2 columns selected after dropna() is empty !')
            else:
        ...
        elif st.session_state.type_plot == 'Histogram':
            fig = go.Figure()
            df_sans_NaN = pd.concat([st.session_state.data[st.session_state.abscisse_plot].reset_index(drop=True),
                                     st.session_state.data[st.session_state.ordonnee_plot].reset_index(drop=True)],
                                    axis=1).dropna()
            ...
            with col3:
                with st.expander("Statistical indices"):
                    st.checkbox("Maximum", key="maximum")
                    st.session_state.moyenne = st.checkbox("Average")
            ...
            if len(df_sans_NaN) == 0:
                ...
                with col4:
                    if st.session_state.type_plot == 'Points' or st.session_state.type_plot == 'Curve':
                        if st.session_state.abscisse_plot not in st.session_state.col_to_time and st.session_state.ordonnee_plot not in st.session_state.col_to_time:
                            with st.expander("Fast regressions"):
                                st.session_state.trendline = st.checkbox("Linear regression")
                                st.session_state.polynom_feat = st.checkbox("Polynomial regression")
                                if st.session_state.polynom_feat:
                                    st.session_state.degres = st.slider('Degrees of polynomial regression',
                                                                        min_value=2,
                                                                        max_value=100)
                ...
                if st.session_state.trendline:
                    ...
                    fig.add_scatter(x=x_range, y=y_range, name='<b>Linear regression<b>', mode='lines',
                                    marker=dict(color='red'))
                    with equation_col_1:
                        with st.expander('Linear regression equation'):
                ...
                if st.session_state.polynom_feat:
                    ...
                    with equation_col_2:
                        with st.expander('Polynomial features equation'):
