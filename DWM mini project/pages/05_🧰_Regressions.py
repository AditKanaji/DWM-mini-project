...
from utils import *
# ===== html/css config ===== #
st.set_page_config(layout="wide", page_title="Regressions")
...
# ===== Page ===== #
st.markdown('<p class="grand_titre">Regressions</p>', unsafe_allow_html=True)
st.write("##")
exp1, exp2, exp3 = st.columns((0.2, 1, 0.2))
with exp2:
    with st.expander("The main regression algorithms"):
        st.write("""
        * Linear regression
...
if 'data' in st.session_state:
    ...
    with exp2:
        st.write("##")
        st.markdown('<p class="section">Selection of data for regression</p>', unsafe_allow_html=True)
    ...
    if st.session_state.choix_target_reg in st.session_state.choix_features_reg:
        ...
        with choix_ensemble_learning:
            st.markdown('<p class="section">choice d\'un algorithm d\'ensemble learning</p>', unsafe_allow_html=True)
            st.session_state.choix_ensemble_learning = st.selectbox("Ensemble learning",
                                                            ENSEMBLE_LEARNING,
                                                            help="Default 50 estimators"
                                                            )
        st.write("##")
        ...
        if len(df_sans_NaN) == 0:
            ...
            try:
                ...
                with box1_title:
                    ...
                    st.write("##")
                    st.write("##")
                    st.markdown('<p class="section">Linear regression</p>', unsafe_allow_html=True)
                    st.write("##")
                # Modèle
                ...
                with box1_eval1:
                    st.metric(label="MSE (compared to the train)", value=round(MSE_reg_test, 3),
                                delta=round(MSE_reg_test - MSE_reg_train, 3), delta_color="inverse")
                with box1_eval2:
                    st.metric(label="RMSE (compared to the train)", value=round(RMSE_reg_test, 3),
                                delta=round(RMSE_reg_test - RMSE_reg_train, 3), delta_color="inverse")
                with box1_eval3:
                    st.metric(label="MAE (compared to the train)", value=round(MAE_reg_test, 3),
                                delta=round(MAE_reg_test - MAE_reg_train, 3), delta_color="inverse")
                with box1_eval4:
                    st.metric(label="r² (compared to the train)", value=round(r2_reg_test, 3),
                ...
                with box2_title:
                    st.write("##")
                    st.markdown('<p class="section">Polynomial regression</p>', unsafe_allow_html=True)
                    st.write("##")
                # Modèle
                ...
                if np.issubdtype(y_train.dtype, int) and np.any(y_train < 0):
                    with box3_title:
                        st.write("##")
                        st.markdown('<p class="section">Poisson regression</p>', unsafe_allow_html=True)
                        st.write("##")
                    # Modèle
                ...
                with box2_title:
                    st.info("Unable to perform regressions with this data")
